...
def Circle_loopProgress(char* fname1, fname2, fname3, int numberOfReactions, win_threshold1, win_threshold2, np.ndarray[DTYPE2_t, ndim=1] reactionrate, par_ini):
    cdef double MT, KT, S, a0,  a1, a2, d1, d2, k0, k1, k2,k3, k4, f1, f2, f_1, f_2, R, X, M, MR, Mp, MpK, K, time, rr[2], step, v,time1
    cdef int N, counter, counter1, i, j,n_c, h, particle, loop
   ...
   # print R, X, MR, Mp, MpK,M
    ...
    File3 = open(fname3,"w")
    File3.write('time, counter, duration, direction \n')
    filename3=''
    entropy=0;
    particle=0;
    K = KT - MpK;
...
################################################################################################################
              if rr[1]*a0<a1*(R)/v and particle==0:   # particle transition from M to MR
                 MR = MR+1;   
                 M = M-1;
                 particle=particle-1;   
                 ...
                 M = M-1;
################################################################################################################
              elif rr[1]*a0<sum_rate[0]+ a2*K/v and (abs(particle)==2):   # particle transition from Mp to MpK
                 Mp = Mp-1;   
                 MpK = MpK+1;     
                 K=K-1;
                 particle=particle-1;      
                 ...
                 K=K-1;
################################################################################################################
              elif rr[1]*a0<sum_rate[1]+ d1 and (particle==3 or particle==-1):   # particle transition from MR to M
                 MR = MR -1;   
                 M=M+1;
                 particle=particle+1;         
                 ...
                 M=M+1;
################################################################################################################ 
              elif rr[1]*a0<sum_rate[2]+ d2 and (particle==-3 or particle==1):   # particle transition from MpK to Mp
                 MpK = MpK -1; 
                 Mp = Mp + 1;
                 K=K+1;
                 particle=particle+1;                   
                 ...
                 K=K+1;
################################################################################################################  
              elif rr[1]*a0<sum_rate[3]+ f1 and (particle==3 or particle==-1):   # particle transition from MR to Mp
                 MR = MR -1;
                 Mp = Mp+1;
                 particle=particle-1;     
                 ...
                 Mp = Mp+1;
################################################################################################################ 
              elif rr[1]*a0<sum_rate[4]+ f_1*(R)/v and (abs(particle)==2):   # particle transition from Mp to MR
                 MR = MR +1;
                 Mp = Mp-1;
                 particle=particle+1; 
                 ...
                 Mp = Mp-1;
################################################################################################################ 
              elif rr[1]*a0<sum_rate[5]+ f_2*K/v  and (particle==0):   # particle transition from M to MpK
                 M = M-1;
                 MpK=MpK+1;
                 K=K-1;
                 particle=particle+1; 
                 ...
                 K=K-1;
################################################################################################################ 
              elif rr[1]*a0<sum_rate[6]+ f2 and (particle==1 or particle==-3):   # particle transition from MpK to M
                 MpK=MpK-1;
                 M=M+1;
                 K=K+1;
                 particle=particle-1; 
